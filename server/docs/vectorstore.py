...
def extract_pdf_text_with_ocr(pdf_path):
    ...
    for page in doc:
        ...
        if len(text) < 50:# make detection more reliable for scanned pages
            ...
            ocr_text = pytesseract.image_to_string(img, lang="eng+hin+ben+tam+tel+ara+guj+urd+mar").strip()
...
async def load_vectorstore(uploaded_files, doc_id: str):
    embed_model = GoogleGenerativeAIEmbeddings(
        model="models/gemini-embedding-001"
    )

    # Clear old vectors ONCE
    ...
    for file in uploaded_files:
        ...
        embeddings = await asyncio.to_thread(
            embed_model.embed_documents, chunks
        )
